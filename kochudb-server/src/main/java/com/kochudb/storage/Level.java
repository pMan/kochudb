...
public class Level {
	...
	public void compactLevel() throws IOException {
		...
		while (! keyValueHeap.isEmpty()) {
			...
			if (curSize >= maxFileSizeInLevel) {
				...
				newSegment = SSTable.createNewSegment(level + 1);
	...
	private void renameIndexFiles() {
		Level next = tree.levels.get(level + 1);
		for (String tempName: LSMTree.filesToRename) {
			String newName = FileUtil.renameIndexFile(tempName);
		}
	...
	private void deleteCompactedFiles() {
		for (Segment s: segments) {
			...
			logger.debug(indexfile.delete() ? "File deleted: {}" : "Failed to delete file: {}", indexfile.getAbsolutePath());
			logger.debug(datafile.delete() ? "File deleted: {}" : "Failed to delete file: {}", datafile.getAbsolutePath());
		}
	}
